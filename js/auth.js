(() => {
  ...
  window.showApp=async()=>{$('auth-container').classList.add('hidden');$('app-container').classList.remove('hidden');try{if(window.loadAll)await window.loadAll();}catch(e){console.error(e);alert(e.message||'Could not load wallet data.');}};
