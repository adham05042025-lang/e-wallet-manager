...
(function () {
    ...
    async function buildReport(month, type) {
        ...
        // Net available is the same cash ledger as the dashboard. Movement cash impact
        // is informational here and must not be added to the net balance a second time.
        const available = salary + collectedClients + adjustment - fixed - recurring - dailyTotal;
        const sections = {
        ...
        const body = type === 'comprehensive' ? `${sections.income}${sections.expenses}${sections.movements}${sections.cashflow}` : (sections[type] || sections.cashflow);
    ...
    function init() {
        document.addEventListener('change', e => { if (e.target?.id === 'report-month' || e.target?.id === 'report-type') renderReport(e); }, true);
        document.addEventListener('click', e => { if (e.target?.closest?.('#btn-generate-report')) renderReport(e); }, true);
