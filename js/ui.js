...
setMobileMenu(false);
document.addEventListener('keydown',e=>{if(e.key==='Escape'&&appSidebar?.classList.contains('mobile-menu-open')){setMobileMenu(false);mobileMenuToggle?.focus()}});
mobileMenuToggle?.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();setMobileMenu(!appSidebar?.classList.contains('mobile-menu-open'))});
mobileNavOverlay?.addEventListener('click',()=>setMobileMenu(false));
navItems.forEach(item=>item.addEventListener('click',()=>{navItems.forEach(n=>n.classList.remove('active'));item.classList.add('active');contentSections.forEach(s=>s.classList.add('hidden'));const id=item.getAttribute('data-target');document.getElementById(id)?.classList.remove('hidden');if(typeof refreshSectionData==='function')refreshSectionData(id);setMobileMenu(false)}));
window.addEventListener('resize',()=>{if(innerWidth>768)setMobileMenu(false)});window.setMobileMenu=setMobileMenu;
...
setupModal('open-add-client-modal','modal-client');setupModal('open-add-task-modal','modal-task');setupModal('open-add-expense-modal','modal-expense');setupModal('open-add-movement-modal','modal-movement');
function closeModal(id){document.getElementById(id)?.classList.add('hidden')}
// Load finance fixes only after every original application script has executed.
window.addEventListener('load',()=>{const s=document.createElement('script');s.src='js/finance-fixes.js?v=20260829-1';document.body.appendChild(s)},{once:true});